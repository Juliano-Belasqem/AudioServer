...
function Install-Dependencies { & python -m pip install -r (Join-Path $ProjectDir 'requirements.txt'); if($LASTEXITCODE -ne 0){ throw 'Falha ao instalar dependencias' } }
...
function Update-Repository {
 ...
 try { Install-Dependencies } catch { Write-Warning 'Falha ao instalar dependencias. Fazendo rollback.'; git reset --hard $previous | Out-Host; throw }
...
try{Install-Dependencies;Update-Repository|Out-Null}catch{Write-Warning "Falha na atualizacao inicial: $($_.Exception.Message)"}
